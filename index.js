const jsonFile = `[{
    ...
    "url": "https://i.pinimg.com/originals/65/a2/53/65a253a2c47f89237b7a0f04e9509b8e.jpg",
...
},
{
    ...
    "url" : "https://avatars.mds.yandex.net/i?id=2cfd4e7ac65c9b21019ed05ed5698614_l-5228318-images-thumbs&n=13",
...
},
{
    ...
    "url" : "https://s9.travelask.ru/system/images/files/000/358/942/wysiwyg/526437_iron-man_nyu-jork_marvel_film_filmy_kino_1920x1200_www.GetBg_.net_.jpg",
...
},
{
    ...
    "url" : "https://avatars.mds.yandex.net/get-mpic/1056698/img_id20348382688180519.jpeg/orig",
...
},
{
    ...
    "url" : "https://i.pinimg.com/originals/7a/d7/15/7ad715227a0707890f92bcb50f1e2d94.jpg",
...
},
{
    ...
    "url" : "https://i.pinimg.com/736x/16/b9/d0/16b9d038a1d96b0ca03ef53cfbf6e55c--thor-marvel-marvel-heroes.jpg",
...
},
{
    ...
    "url" : "https://i.pinimg.com/736x/a8/cb/eb/a8cbeb663f140f51c93e01ec6a0684a4.jpg",
...
},
{
    ...
    "url" : "https://avatars.mds.yandex.net/i?id=2a00000179edff17013ff836202730c487db-4438614-images-thumbs&n=13",
...
},
{
    ...
    "url" : "https://comicvine.gamespot.com/a/uploads/original/11134/111341901/8283349-9626021636-black.jpg",
...
},
{
    ...
    "url" : "https://i.pinimg.com/736x/89/78/15/897815f4e44975537951233d831bafad.jpg",
...
//Добавляем информацию из JSON файла в карточки HTML
document.addEventListener("DOMContentLoaded", function(event) {
    const superHeroes = JSON.parse(jsonFile);

    const frontArray = document.querySelectorAll('.card__front');
    for (let i = 0; i < frontArray.length; i++) {
        frontArray[i].innerHTML = `<div class="sh-card">
        <h2>${superHeroes[i].name}</h2>
        <p>${superHeroes[i].universe}</p>
        <p>Альтер-эго: ${superHeroes[i].alterego}</p>
        <p>${superHeroes[i].occupation}</p>
        <p>Друзья: ${superHeroes[i].friends}</p>
        <p>Суперсила: ${superHeroes[i].superpowers}</p>
        <img src=${superHeroes[i].url} width=300px height=250px/>
        </div>`;
    }
    const backArray = document.querySelectorAll('#info');
    for (let i = 0; i < backArray.length; i++) {
        backArray[i].innerHTML = `<p class="sh-card__back">${superHeroes[i].info}</p>`;
    }
});

//Сохраняем рейтинг персонажей в localStorage
const heroes = [
    'batman',
    'superman',
    'ironman',
...
function addEventListeners(hero) {
    const inputs = document.querySelectorAll(`input[name=${hero}_rating]`);
    inputs.forEach(input => { 
    input.addEventListener('change', () => { 
        if(input.checked) { 
        localStorage.setItem(`${hero}_rating`, input.value); 
        } 
    });
    }); 
    const savedRating = localStorage.getItem(`${hero}_rating`);
    if (savedRating) { 
    inputs.forEach(input => {
...
heroes.forEach(hero => addEventListeners(hero));